...
const transSmeformResponse = require("../../helpers/jsonSmeFormatter")
...
class ProcessPickwaveSmeBillsPayment {
  /**
    ...
    try {
      ...
      if (response.code != "101") {
        await refundFailed.processFailedPayment(
          ...
          fee,
          billstransactions,
          response?.description.ReferenceID || null,
          response?.description.ReferenceID || null
        );
    
        throw new Error(response.description.message);
      }
      ...
      transaction.transactionID = response?.description.ReferenceID || null;
      transaction.requestId = response?.description.ReferenceID || null;
      ...
      let notification = {
        ...
        message:
          ...
          response.description.ReferenceID +
      ...
      console.log("from transform "+transSmeformResponse(response,totalamount,billstransactions.desc))
     ...
     // transformResponse(response,fee)
      ...
      return { data: transSmeformResponse(response,totalamount,billstransactions.desc), wallet, transaction, status: "success" };
    } catch (error) {
...
module.exports = new ProcessPickwaveSmeBillsPayment();
