...
const requeryBillsTransactionQueue = require("../../utils/queue/RR.JS");
const transformResponse = require("../../helpers/jsonElectricFormatter")
  ...
  ProcessElectricBill = async (
    ...
    try {
      ...
      if (response.responseCode != "0" && response.responseCode != "3") {
        console.log("Error triggered here failed")
      ...
      if (response.responseCode == "3") {
        const requeryData = {
          tid,
          request_id: paymentDetails.referenceNumber,
          apiUrl: `https://www.mypaga.com/paga-webservices/business-rest/secured/transactionStatus`,
          method,
          headers,
          walletId,
          fee,
      ...
      transaction.requestId = response.referenceNumber;
      transaction.token = response.merchantTransactionReference || null;
      await transaction.save({ session });
      const updatedWallet = await walletService.getWalletByWalletId(walletId);
      ...
      let notification = {
        ...
        message:
          ...
          response.referenceNumber +
          " has been processed successfully."+ " Light Token: "+response.merchantTransactionReference,
      ...
      return { data: transformResponse(response,fee) , wallet, transaction, status: "success" };
    } catch (error) {
...
module.exports = new ProcessElectricBill();
