...
const { Product } = require("../models/Product"); // Adjust path to your Listing model
const mongoose = require("mongoose"); // Only needed if you connect DB here, otherwise connect in app.js

const ONE_HOUR = 60 * 60 * 1000; // 1 hour in milliseconds

const expiredProductFeaturedListingJobs = async () => {
  console.log("--- Running product featured listings expiry job ---");
  ...
  try {
    ...
    if (expiredListings.length === 0) {
      console.log("No featured product listings to expire at this time.");
      return;
    }
...
const startProdFeaturedListingCronJob = () => {
  ...
  cron.schedule("0 * * * *", () => {
    ...
    expiredProductFeaturedListingJobs();
  });

  console.log("Featured  product listings expiry cron job scheduled.");
...
module.exports = {
  ...
  expiredProductFeaturedListingJobs, // Export for potential manual triggering or testing
};
