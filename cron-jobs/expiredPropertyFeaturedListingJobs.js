...
const Property = require("../models/Property"); // Adjust path to your Listing model
const mongoose = require("mongoose"); // Only needed if you connect DB here, otherwise connect in app.js

const ONE_HOUR = 60 * 60 * 1000; // 1 hour in milliseconds

const expiredPropertyFeaturedListingJobs = async () => {
  console.log("--- Running property featured listings expiry job ---");
  ...
  try {
    ...
    if (expiredListings.length === 0) {
      console.log("No featured property listings to expire at this time.");
      return;
    }
...
const startPropFeaturedListingCronJob = () => {
  ...
  cron.schedule("0 * * * *", () => {
    ...
    expiredPropertyFeaturedListingJobs();
  });

  console.log("Featured property listings expiry cron job scheduled.");
...
module.exports = {
  ...
  expiredPropertyFeaturedListingJobs, // Export for potential manual triggering or testing
};
