...
const sendBalanceUpdate = (walletId, newBalance) => {
  if (!io) {
    console.error("Socket.io not initialized");
    return;
  }
  console.log("called: socketIO.sendBalanceUpdate", newBalance);
  // Emit the balance update to the specified wallet's connected client(s)
  io.emit("balanceUpdate", { walletId, newBalance });
};


module.exports = { initSocket, sendBalanceUpdate };
